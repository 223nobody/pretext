...
import { Eraser, Film, SlidersHorizontal } from "lucide-react";
...
export function BackgroundControls() {
  ...
  return (
    <div className="control-stack">
      ...
      <label className="control-row">
        <span>{t(language, "mask")}</span>
        <input
          type="range"
          min="120"
          max="420"
          value={backgroundVideo.maskSize}
          onChange={(event) => setBackgroundMask({ size: Number(event.target.value) })}
        />
        <output>{backgroundVideo.maskSize}px</output>
      </label>
