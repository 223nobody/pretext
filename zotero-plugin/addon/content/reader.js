...
(() => {
  ...
  // src/pretext-bridge.ts
  async function renderReader(payload, documentRef = document) {
    try {
      await Promise.resolve().then(() => (init_layout(), layout_exports));
    } catch {
    }
    ...
    if (meta) {
      ...
      meta.textContent = [authors, sources].filter(Boolean).join(" \xB7 ");
    }
    if (reader) {
  ...
  // src/zotero-api.ts
  async function getSelectedReaderPayload() {
  ...
  function isPdfAttachment(attachment) {
    return Boolean(
      attachment.isPDFAttachment?.() || attachment.attachmentContentType === "application/pdf" || attachment.getField?.("contentType") === "application/pdf"
    );
  }
  async function readIndexedAttachmentText(attachment) {
    try {
      ...
      const text = await fulltext.getItemText?.(itemId) ?? await fulltext.getItemCacheFileContents?.(itemId) ?? "";
      return typeof text === "string" ? text.trim() : "";
    } catch {
      return "";
    }
  }
  async function readAttachmentFileText(attachment) {
    try {
  ...
  // src/ui.ts
  var PretextReaderPlugin = class {
    constructor(options) {
      __publicField(this, "rootURI");
      __publicField(this, "menuItemId", "pretext-reader-open-item");
      __publicField(this, "toolbarButtonId", "pretext-reader-toolbar-button");
